...
class TestUserAuthView(TestCase):
    ...
    def setUp(self):
        self.user1 = User.objects.create(
            email=user["email"], first_name=user["first_name"], last_name=user["last_name"], is_superuser=user["is_superuser"], photo=user["photo"])
    ...
    def test_user_can_signup(self):
        ...
        with open(finders.find("images/photo.png"), "rb") as profile_image:
            response = self.c.post(reverse("accounts:signup"), {
    ...
    def test_user_can_login(self):
        ...
        self.assertEqual(int(self.c.session["_auth_user_id"]), User.objects.get(
            email=user["email"]).id)
    ...
    def test_user_can_change_password(self):
        ...
        """
        self.c.login(email=user["email"], password=user["password"])
        response = self.c.post(reverse("accounts:change-password"), {
    ...
    def test_user_can_update_profile(self):
        ...
        self.c.login(email=user["email"], password=user["password"])
        with open(finders.find("images/photo.png"), "rb") as profile_image:
            response = self.c.post(reverse("accounts:profile-update"), {
                "first_name": "ABC",
                "middle_name": "MNOP",
                "last_name": "XYZ",
                "gender": "F",
        ...
        self.assertTrue(User.objects.filter(first_name="ABC",
                        middle_name="MNOP", last_name="XYZ").exists())
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(response, reverse("accounts:profile-view"))
