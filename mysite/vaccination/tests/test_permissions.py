...
class TestPermissionsOnVaccinationView(TestCase):
    ...
    def test_unauthorized_access_on_vaccination_list_view_for_patient(self):
        response = self.c.get(reverse("vaccination:vaccination-list-patient"))
        self.assertEqual(response.status_code, 403)

    def test_authorized_access_on_vaccination_list_view_for_patient(self):
        self.user.user_permissions.add(
            Permission.objects.get(codename="view_vaccination"))
        response = self.c.get(reverse("vaccination:vaccination-list-patient"))
        self.assertEqual(response.status_code, 200)
